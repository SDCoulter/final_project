...
* `ML - KMeans Clustering Analysis of Dataset (Drop LL)` - [here](notebooks/machine_learning/ML%20-%20KMeans%20Clustering%20Analysis%20DROP%20LL.ipynb) - in this notebook we explore KMeans clustering by again providing the dataset from the SQL database, removing the null rows and unneeded columns. We also drop the Life Ladder values too, so we can compare the changes in the clustering over the years to the changes in Life Ladder values from the actual dataset. We fit all the data to one KMeans model, and then predict the clusters for each country for each year, allowing the model to provide similar values for a country's environmental cluster over time. We use 4 clusters as that seemed to return the best analysis when we did earlier versions of this modelling. The data is scaled and transformed as in the Neural Network analysis. We store the classes for each country into a new table for each year and export it for further visualization use - Tableau.
...
* `segment1/`, `segment2/`, and `/segment3` - the analysis that led up to our more polished machine learning models.
